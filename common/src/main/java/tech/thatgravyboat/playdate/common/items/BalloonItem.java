...
public class BalloonItem extends Item implements DyeableItem {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (world instanceof ServerWorld server) {
            BalloonEntity balloonEntity = ModEntities.BALLOON.get().create(server, null, null, user, user.getBlockPos().add(0, 3, 0), SpawnReason.TRIGGERED, false, false);
            ...
            if (balloonEntity != null) {
                ...
                user.setStackInHand(hand, ItemUsage.exchangeStack(stack, user, new ItemStack(ModItems.ROPE.get())));
            }
        }
